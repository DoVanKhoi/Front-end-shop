        ...
        <div className="my-4 px-4 bg-white rounded">
            <div className="grid gap-x-6 gap-y-10 lg:grid-cols-6 md:grid-cols-4 sm:grid-cols-2 xl:gap-x-8">
                {filteredProducts?.slice(0, visibleCount).map((product) => (
                    <div key={product?._id} className={`relative border-2 border-slate-300 rounded ${product?.countInStock === 0 ? 'opacity-70' : ''}`}>
                        {product?.countInStock === 0 && (
                            <div className="absolute inset-0 flex items-center justify-center bg-white bg-opacity-75">
                                <span className="text-lg font-bold text-red-600">Hết hàng</span>
                            </div>
                        )}
                        <button
                        ...
                        >
                            <img
                                alt=""
                                src={product?.image || ""}
                                className="aspect-square w-full rounded-t-sm bg-gray-200 object-cover group-hover:opacity-75 xl:aspect-[7/8]"
                            />
